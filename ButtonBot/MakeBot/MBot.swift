...
open class MBot: MakeblockRobot {
    ...
    /// which LED Light to set when sending setRGBLED commands
    public enum RGBLEDBoardPosition: UInt8 {
        case first = 0x01
        case second = 0x02
        case third = 0x03
        case fourth = 0x04
        
        public static let all: [RGBLEDBoardPosition] = [
            .first,
            .second,
            .third,
            .fourth
    ...
    open func setMotors(_ leftMotor: Int, rightMotor: Int) {
        ...
        _ = sendMessage(.dcMotorMove, arrayOfBytes: [leftLow, leftHigh, rightLow, rightHigh])
    ...
    open func setMotor(_ port: MBotPorts, speed: Int) {
        ...
        _ = sendMessage(.dcMotor, arrayOfBytes: [port.rawValue, low, high])
    }
    ...
    open func moveForward(_ speed: Int) {
        self.setMotors(-speed, rightMotor: speed)
    }
    ...
    open func stopMoving() {
        self.setMotors(0, rightMotor: 0)
    }

    /**
     Tell the mBot to start or stop the bubbles
     */
    open func setBubbles(_ value: Bool) {
        _ = sendMessage(value ? .pwm : .digital, arrayOfBytes: [UInt8(255)])
    }

    /**
     Set the servo

     - parameter port:  which port the servo is connected
     - parameter speed: the speed of the motor 0 - 180
     */
    open func setServo(_ port: MBotPorts, value: Int) {
        _ = sendMessage(.servo, arrayOfBytes: [port.rawValue, 0x02, UInt8(value)])
    }

    /**
     Set the color of daughter-board LEDs of the mBot
     ...
     */
    open func setRGBLED(_ port: MBotPorts, position: RGBLEDBoardPosition, red: Int, green: Int, blue: Int) {
        print("\(position.rawValue) set to r:\(red),g:\(green),b:\(blue)")
        
        _ = sendMessage(.rgbled, arrayOfBytes: [
            port.rawValue,
            0x02,
            position.rawValue,
            UInt8(red),
    ...
    open func clearRGBLED(_ port: MBotPorts) {
    ...
    open func setBuzzer(_ pitch: MusicNotePitch, duration: MusicNoteDuration) {
        ...
        _ = sendMessage(.buzzer, arrayOfBytes: [pitchLow, pitchHigh, durationLow, durationHigh])
    }
    ...
    open func getUltrasonicSensorValue(_ port: MBotPorts = .port3, callback: @escaping ((Float) -> Void)) {
        _ = sendMessage(.ultrasonicSensor, arrayOfBytes: [port.rawValue]) { value in
    ...
    open func getLightnessSensorValue(_ port: MBotPorts = .lightnessSensor, callback: @escaping ((Float) -> Void)) {
        _ = sendMessage(.lightnessSensor, arrayOfBytes: [port.rawValue]) { value in
            callback(value.floatValue)
        }
    }
    ...
    open func getLinefollowerSensorValue(_ port: MBotPorts = .port2, callback: @escaping ((LineFollowerSensorStatus) -> Void)) {
        _ = sendMessage(.lineFollowerSensor, arrayOfBytes: [port.rawValue]) { value in
            callback(LineFollowerSensorStatus(rawValue: value.floatValue)!)
        }
    }
